...
function getApiKey(): string {
	...
	if (!key) {
		...
		throw new ApiError(
			"RUNPOD_API_KEY or BEARER environment variable required",
		);
	}
	return key;
}
...
export function validateApiResponse(data: unknown): data is ApiResponse {
	...
	if (typeof response.executionTime !== "number") {
		validationErrors.push(
			"API response missing or invalid 'executionTime' field",
		);
	}
	...
	if (typeof response.output !== "object" || response.output === null) {
		...
		if (typeof output.audio_url !== "string") {
			validationErrors.push(
				"API response missing or invalid 'output.audio_url' field",
			);
		}
		...
		if (typeof output.cost !== "number") {
			validationErrors.push(
				"API response missing or invalid 'output.cost' field",
			);
		}
	}
...
export async function callTtsApi(
	...
	for (let attempt = 1; attempt <= API_MAX_RETRIES; attempt++) {
		...
		try {
			...
			if (
				err instanceof ApiResponseValidationError &&
