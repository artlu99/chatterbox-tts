...
import { ChatterboxError, ValidationError } from "./errors.ts";
...
function parseArgs(): { voice: string; outDir: string } {
	...
	for (let i = 0; i < args.length; i++) {
		const arg = args[i];
		const next = args[i + 1];
		invariant(typeof next === "string", "Voice parameter must be a string");
...
function validateInput(text: string, voice: string): void {
	...
	if (voice.length === 0) {
		throw new ValidationError(
			"Voice parameter cannot be empty",
			"voice",
			voice,
		);
	}
	...
	if (voice.length > 100) {
		throw new ValidationError(
			"Voice parameter too long (max 100 characters)",
			"voice",
			voice,
		);
	}
